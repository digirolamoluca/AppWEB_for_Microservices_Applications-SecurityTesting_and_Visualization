...
def dockercompose_WriteTo_macmFile(output_folder,dockercompose_path, app_id, application_name, xtype, xdevtype):

	file_cypher=open(output_folder+application_name+app_id+".macm","w")
	file_cypher.write("CREATE\n")

	#creo grafo
	macm_graph=nx.DiGraph()
	...
	VM=macm.Node("IaaS","service","VM1","VM","","","")
	object_node.append(VM)
	macm_graph.add_node(VM)
	Docker=macm.Node("CaaS","service","Docker","?","","","")
	object_node.append(Docker)
	macm_graph.add_node(Docker)

	#se c'è qualche container privo di networks creo nodo default-network
	if(len(networks_connects) != len(container_name)):
		default_network=macm.Node("network","network","default-network","?","","","")
		object_node.append(default_network)
		macm_graph.add_node(default_network)

	#creato i nodi container
	ind=0
	for x in container_name:
		node_container.append(macm.Node("SaaS","service",x,asset_type[ind],"","",""))
		object_node.append(node_container[ind])
		macm_graph.add_node(node_container[ind])
		ind+=1
	...
	if(len(networks_connects)!=0):
		ind=0
		for x in networks:
			node_networks.append(macm.Node("network","network",x,"?","","",""))
			object_node.append(node_networks[ind])
			macm_graph.add_node(node_networks[ind])
			ind+=1
			...
			for key in docker_composeObj['services'][x].keys():
				...
				if(key=='depends_on'):
					...
					for i in range(0,len(depends_on[ind]),1):
						...
						edge_uses.append(macm.Edge(node_container[count],to_node,"uses"+protocol_type[count]))
						object_edge.append(edge_uses[count_edge])
						macm_graph.add_edge(node_container[count],to_node,attribute="uses"+protocol_type[count])
						count_edge+=1
	...
	for j in range(0,len(net),1):
		if(net[j]==0):
			edge_connects_default_networks.append(macm.Edge(node_container[j],default_network,"connects"))
			object_edge.append(edge_connects_default_networks[count])
			macm_graph.add_edge(node_container[j],default_network,attribute="connects")
			count+=1
			...
			for key in docker_composeObj['services'][x].keys():
				if(key=='networks'):
					...
					for i in range(0,len(networks_connects[ind]),1):
						...
						edge_connects.append(macm.Edge(node_container[count],to_node,"connects"))
						object_edge.append(edge_connects[count_edge])
						macm_graph.add_edge(node_container[count],to_node,attribute="connects")
						count_edge+=1
					ind+=1
			count+=1
	...
	edge=macm.Edge(VM,Docker,"hosts")
	object_edge.append(edge)
	macm_graph.add_edge(VM,Docker,attribute="hosts")
	...
	for i in range(0,len(container_name),1):
		...
		edge_hosts.append(macm.Edge(Docker,to_node,"hosts"))

		object_edge.append(edge_hosts[count_edge])
		macm_graph.add_edge(Docker,to_node,attribute="hosts")
		count_edge+=1
	...
	if(xtype==0 and xdevtype==0):
		for i in range(0,len(object_node),1):
			file_cypher.write("\t("+filtering(object_node[i].get_name())+":"+object_node[i].get_secondary()+":"+object_node[i].get_primary()+" {name:'"+object_node[i].get_name()+"', type:'"+object_node[i].get_type()+"', dev_type:'"+dev_type[i]+"', app_id:'"+app_id+"', application_name:'"+application_name+"'}),\n")
		file_cypher.write("\n")
	...
	if(xtype!=0 and xdevtype!=0):
		for i in range(0,len(object_node),1):
			file_cypher.write("\t("+filtering(object_node[i].get_name())+":"+object_node[i].get_secondary()+":"+object_node[i].get_primary()+" {name:'"+object_node[i].get_name()+"', type:'"+xtype[i]+"', dev_type:'"+xdevtype[i]+"', app_id:'"+app_id+"', application_name:'"+application_name+"'}),\n")
		file_cypher.write("\n")
		for i in range(0,len(object_edge),1):
			#if per la virgola finale da omettere nel macm
			if(i!=len(object_edge)-1):
