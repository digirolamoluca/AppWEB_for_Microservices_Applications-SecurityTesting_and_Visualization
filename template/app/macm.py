...
def dockercompose_WriteTo_macmFile(dockercompose_path, app_id, application_name):
	...
	for i in range(0,len(object_node),1):
		file_cypher.write("\t("+filtering(object_node[i].get_name())+":"+object_node[i].get_label()+" {name:'"+object_node[i].get_name()+"', type:'"+object_node[i].get_type()+"', app_id:'"+app_id+"', application:'"+application_name+"'}),\n")
	file_cypher.write("\n")
